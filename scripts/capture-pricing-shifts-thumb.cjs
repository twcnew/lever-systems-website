...
const OUT_PNG = join(ROOT, "public", "studio", "pricing-shifts.png");
const RAW_PNG = join(tmpdir(), "lever-pricing-shifts-thumb-raw.png");
const PAGE_URL =
  process.env.PRICING_SHIFTS_URL ??
  "http://localhost:3001/studio/pricing-shifts/";
const TARGET_W = Number(process.env.PRICING_SHIFTS_WIDTH) || 720;
...
async function main() {
  ...
  await page.addStyleTag({
    content: `
      ...
      .studio-spine__poster--pricing-shifts {
  ...
  const poster = page.locator(".studio-spine__poster--pricing-shifts");
