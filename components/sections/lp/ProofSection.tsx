...
function annotatedTitleAccent(titleAccent: string) {
  ...
  const after = titleAccent.slice(
    titleAccent.indexOf(PROOF_TITLE_ACCENT_ANNOTATED) + PROOF_TITLE_ACCENT_ANNOTATED.length,
  );
...
function annotatedSub(sub: string) {
  ...
  const after = sub.slice(sub.indexOf(PROOF_SUB_ANNOTATED) + PROOF_SUB_ANNOTATED.length);
